# -*- coding: utf-8 -*-
"""
S_Acc Evaluation Framework for Traditional Chinese Idioms
...
ORIGINAL_FILE = os.environ.get(
    "ORIGINAL_FILE",
    r"data/reference_explanations/traditional_chinese_explanations.csv"
)

# Input and output files controlled by batch_baseline.py
MODEL_FILE = os.environ.get("MODEL_FILE", "")
OUTPUT_FILE = os.environ.get("OUTPUT_FILE", "results/sacc_traditional_chinese_output.csv")
...
BATCH_SIZE_EMB = 64
USE_BERTSCORE = True
BERTSCORE_LANG = 'zh_trad'  # Traditional Chinese
MAX_LEN = 512
...
# ===== Stopwords/Polarity Words (Traditional Chinese) =====
STOPWORDS: Set[str] = set("的了呢啊吧哦恩嗯亦也又與且在及或並被把對給而其之於") | \
    {"或者", "以及", "還有", "一種", "對於", "進行", "由於", "通過", "因此", "例如", "比如", "同時", "這", "是", "個", "來", "去", "為"}

NEGATIONS: Set[str] = {"不", "未", "無", "非", "別", "從不", "毫不", "不再", "不願", "沒", "沒有", "不可", "難以", "未曾", "從未", "並非", "不是", "無法", "禁止", "否定", "反"}

POLAR_POS: Set[str] = {"喜愛", "喜歡", "愛慕", "鍾愛", "偏愛", "珍愛", "喜好", "愛不釋手", "捨不得", "不捨",
                       "珍視", "讚美", "欣賞", "褒揚", "優秀", "出色", "崇高", "優點", "優勢", "肯定", "支持"}

POLAR_NEG: Set[str] = {"厭惡", "輕視", "不喜歡", "不珍惜", "捨棄", "拋棄", "丟棄", "貶低", "批評", "僵化", "死板", "落後",
                       "頑固", "不思進取", "空洞", "不正派", "失敗", "倒退", "衰落"}

IDIOM_POLARITY_OVERRIDES: Dict[str, Dict[str, bool]] = {
    "愛不釋手": {"pos": True,  "neg": False},
    "循規蹈矩": {"pos": False, "neg": True},
}
...
def bertscore_over_refs(pred: str, refs: List[str], lang: str = 'zh_trad',
