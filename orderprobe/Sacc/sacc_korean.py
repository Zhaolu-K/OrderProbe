# -*- coding: utf-8 -*-
"""
S_Acc Evaluation Framework for Korean Idioms
...
ORIGINAL_FILE = os.environ.get(
    "ORIGINAL_FILE",
    r"data/reference_explanations/korean_explanations.csv"
)

# Input and output files controlled by batch_baseline.py
MODEL_FILE = os.environ.get("MODEL_FILE", "")
OUTPUT_FILE = os.environ.get("OUTPUT_FILE", "results/sacc_korean_output.csv")

# ===== Device Configuration =====
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'

# ===== Model Configuration =====
# Use lightweight multilingual model for acceleration (MiniLM faster than RoBERTa by 3-5x)
EMBED_MODEL_NAME = 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'
CROSS_ENCODER_MODEL = 'BAAI/bge-reranker-base'

BATCH_SIZE_EMB = 64
USE_BERTSCORE = True
BERTSCORE_LANG = 'ko'  # Korean
MAX_LEN = 512
...
# ===== Stopwords/Polarity Words (Korean) =====
STOPWORDS: Set[str] = {
    "은", "는", "이", "가", "을", "를", "에", "에서", "에게", "와", "과", "도", "만", "으로", "부터", "까지",
    "그리고", "그러나", "하지만", "또는", "또", "또한", "그래서", "그러므로", "때문에",
    "이것", "그것", "저것", "여기", "거기", "저기", "것", "수", "등", "등등",
}

NEGATIONS: Set[str] = {
    "안", "못", "아니", "않", "말", "못하다", "하지", "않다", "없다", "아니다", "불가능", "금지", "부정", "반대",
}

POLAR_POS: Set[str] = {
    "좋다", "좋은", "좋아하다", "사랑", "사랑하다", "사랑스러운", "기쁘다", "행복하다", "만족", "만족하다",
    "훌륭하다", "멋지다", "뛰어나다", "장점", "이점", "강점", "존경", "존중", "칭찬", "지지", "지원", "긍정적",
}

POLAR_NEG: Set[str] = {
    "싫다", "싫은", "싫어하다", "미워하다", "증오", "혐오", "불만", "불만족", "나쁘다", "나쁜",
    "열등하다", "부족하다", "비판", "비난", "부정적", "실패", "후퇴", "퇴보", "타락",
}

IDIOM_POLARITY_OVERRIDES: Dict[str, Dict[str, bool]] = {
    # 일취월장: Progressing day by day (positive)
    "일취월장": {"pos": True,  "neg": False},
    # 자업자득: Reaping what one sows (mostly negative)
    "자업자득": {"pos": False, "neg": True},
}
...
def bertscore_over_refs(pred: str, refs: List[str], lang: str = 'ko',
