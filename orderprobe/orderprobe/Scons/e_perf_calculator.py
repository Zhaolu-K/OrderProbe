# -*- coding: utf-8 -*-
"""
E_perf Calculator - Performance Deviation

Computes E_perf (Performance Deviation) as part of structural consistency assessment.
E_perf measures the average gap between recognition capability and realized performance across all permutations.
...
3. Average these gaps across all arrangements
"""
...
def calculate_performance_deviation(mean_file: Path, max_file: Path) -> float:
    """Calculate E_perf (Performance Deviation) for a single file pair."""
    # Read Excel files
    try:
    ...
    # E_perf = average of (S_max - S_mean) across all arrangements
    e_perf = np.mean(arrangement_gaps)
    return e_perf
...
def main():
    """Main function."""
    print("=" * 60)
    print("E_perf Calculator - Performance Deviation")
    print("=" * 60)
    ...
    for mean_file in sorted(mean_files):
        ...
        # Calculate E_perf
        e_perf = calculate_performance_deviation(mean_file, max_file)

        if not pd.isna(e_perf):
            results.append({
            ...
            print(f"[WARN] Failed to calculate E_perf for {model_name}")
    ...
    # Save results
    output_file = OUTPUT_DIR / "e_perf_results.csv"
