# Information Density (S_Info) Calculator
...
Information Density addresses the "knowledge dumping" problem where models generate excessively verbose explanations to maximize keyword coverage. S_Info encourages models to prioritize precise reasoning over redundant generation.
...
**Formula**:
```
BP = 1                    if len(prediction) > len(reference)
    = exp(1 - r/c)        if len(prediction) ≤ len(reference)
```
...
For integration with other metrics (S_Acc, S_Log, S_Cons), combine S_Info with complementary evaluation approaches for comprehensive idiom explanation assessment.
