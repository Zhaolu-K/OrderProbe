# -*- coding: utf-8 -*-
"""
S_Acc Evaluation Framework for Japanese Idioms
...
ORIGINAL_FILE = os.environ.get(
    "ORIGINAL_FILE",
    r"data/reference_explanations/japanese_explanations.csv"
)

# Input and output files controlled by batch_baseline.py
MODEL_FILE = os.environ.get("MODEL_FILE", "")
OUTPUT_FILE = os.environ.get("OUTPUT_FILE", "results/sacc_japanese_output.csv")

# ===== Device Configuration =====
DEVICE = 'cuda' if torch.cuda.is_available() else 'cpu'

# ===== Model Configuration =====
# Use lightweight multilingual model for acceleration (MiniLM faster than base by 3-5x)
EMBED_MODEL_NAME = 'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'
CROSS_ENCODER_MODEL = 'BAAI/bge-reranker-base'

BATCH_SIZE_EMB = 64
USE_BERTSCORE = True
BERTSCORE_LANG = 'ja'  # Japanese
MAX_LEN = 512
...
# ===== Stopwords/Polarity Words (Japanese) =====
STOPWORDS: Set[str] = {
    "の", "こと", "これ", "それ", "あれ", "ため", "よう", "から", "まで", "など", "でも",
    "に", "は", "が", "を", "と", "や", "も", "へ", "で", "ね", "よ", "な", "さ", "せ",
    "です", "ます", "いる", "ある", "いること", "あること",
}

NEGATIONS: Set[str] = {
    "ない", "ぬ", "ん", "じゃない", "ではない", "なく", "ません", "できない", "無い", "非", "不", "禁止", "否定",
}

POLAR_POS: Set[str] = {
    "好き", "大好き", "愛する", "愛している", "素晴らしい", "良い", "良さ", "優れている", "素敵",
    "尊敬", "敬意", "褒める", "称賛", "賞賛", "長所", "利点", "強み", "支持", "肯定的", "好意的",
}

POLAR_NEG: Set[str] = {
    "嫌い", "大嫌い", "憎む", "ひどい", "悪い", "悪さ", "劣っている", "批判", "非難", "嫌悪",
    "失敗", "後退", "退化", "堕落", "否定的", "マイナス",
}

IDIOM_POLARITY_OVERRIDES: Dict[str, Dict[str, bool]] = {
    # 一心不乱: Concentrating on one thing without distraction (positive)
    "一心不乱": {"pos": True,  "neg": False},
    # 自業自得: Reaping what one sows (mostly negative)
    "自業自得": {"pos": False, "neg": True},
}
...
def bertscore_over_refs(pred: str, refs: List[str], lang: str = 'ja',
