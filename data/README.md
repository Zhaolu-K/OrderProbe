...
Each language corpus is systematically categorized into six major syntactic structures to enable fine-grained structural analysis. The classification provides the foundation for OrderProbe's structural robustness evaluation.

### Structure Types
The six major syntactic categories are:

1. **Parallel (Coordinate)**: Structures with balanced parallel elements
2. **Subject-Predicate**: Traditional subject-verb constructions
3. **Verb-Object**: Action-object relationship structures
4. **Modifier-Head**: Attributive modification patterns
5. **Serial Verb**: Multiple verb constructions
6. **Idiomatic Compounds**: Specialized fixed expressions

### Structure Files
The `Structure/` directory contains language-specific structure annotations:

```
Structure/
├── Simplified Chinese/
│   └── Simplified Chinese.csv    # zh-CN structure labels
├── Traditional Chinese/
│   └── Traditional Chinese.csv   # zh-TW structure labels
├── Japanese/
│   └── Japanese.csv             # ja structure labels
└── Korean/
│   └── Korean.csv               # ko structure labels
```

**Format**: `expression,structure_type`
